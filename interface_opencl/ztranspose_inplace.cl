...
__kernel void ztranspose_inplace_even_kernel( __global magmaDoubleComplex *matrix, int offset, int lda, int half_val )
{
    ...
    int ibx = bottom ? (get_group_id(0) - 1) : (get_group_id(1) + half_val);
    int iby = bottom ? (get_group_id(1))     : (get_group_id(0) + half_val);
...
__kernel void ztranspose_inplace_odd_kernel( __global magmaDoubleComplex *matrix, int offset, int lda, int half_val )
{
    //__local magmaDoubleComplex a[NB][NB+1];
    //__local magmaDoubleComplex b[NB][NB+1];
    ...
    bool bottom = ( get_group_id(0) >= get_group_id(1) );
    int ibx = bottom ? get_group_id(0)  : (get_group_id(1) + half_val - 1);
    int iby = bottom ? get_group_id(1)  : (get_group_id(0) + half_val);
