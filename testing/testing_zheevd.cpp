...
int main( int argc, char** argv)
{
    ...
    for(i=0; i<8; i++){
        ...
        lwork  = std::min( lwork,  (magma_int_t) MAGMA_Z_REAL( h_work[0] ));
        lrwork = std::min( lrwork, (magma_int_t) rwork[0] );
        liwork = std::min( liwork, iwork[0] );
        ...
        if ( checkres ) {
          ...
          temp1 = temp2 = 0;
          for(int j=0; j<N; j++){
            temp1 = std::max(temp1, absv(w1[j]));
            temp1 = std::max(temp1, absv(w2[j]));
            temp2 = std::max(temp2, absv(w1[j]-w2[j]));
          }
